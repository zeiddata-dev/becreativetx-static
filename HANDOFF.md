...
- **Apparel is links, not tiles:** primary link goes to the external branded
  apparel store `https://www.companycasuals.com/BeCreativeOnline/start.jsp`
  (new tab), secondary is `request-a-quote.html`. Edit in `gallery-data.js`.
...
**Not yet wired:** nothing in the site nav links to `gallery.html` yet — it is a
standalone page. Decide where it goes in the header menu.
...
1. **Wire the site nav to `gallery.html`** — decide where the gallery link sits
   in the header menu, then add it across pages.
2. **Review the four `FLAG` type tags** in `casestudies-data.js` (Momentum
   napkins, Momentum laser-cut invitation, Box coverage display, Box 5-year
   award) and confirm or reclassify.
3. **Confirm the Apparel links.** Primary is the Company Casuals store; second
   is currently `request-a-quote.html`. Swap the second if the live site uses a
   different target.
4. **Promo/Design coverage.** `general/promo` and `general/apparel` are empty;
